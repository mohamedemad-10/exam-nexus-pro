...
serve(async (req) => {
  ...
  try {
    ...
    // Verify the requesting user is an admin (only admins can create teachers)
    ...
    if (!roleData) {
      return new Response(JSON.stringify({ error: "Admin access required to create teachers" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    console.log("Teacher created successfully:", userData.user?.id);
